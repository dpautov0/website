...
(function () {
  ...
  const oc = [...bbIn('1' + V),
    ...
    ['vlab', [4.3, 0.4], [4.3, 1.6], { n: 'V_{OC}', s: '2' + V }]];
  const sc = [...bbIn('2' + V),
    ['w', [3.3, 0], [4.3, 0], [4.3, 2], [3.3, 2]],
    ['iarr', [4.3, 1], 'd', { n: 'I_{SC}', s: '1\\,\\text{A}', side: 'r' }]];
  const th = [
  ...
  const dcX = FIGS.shuntX().map((e) => {
    if (e[0] === 'Vac') return ['w', e[1], e[2]];
    const o = e.find((x) => x && typeof x === 'object' && !Array.isArray(x));
    if (!o) return e;
    const n = { 'V_{OUT}+v_{out}': 'V_{OUT}', i_X: 'I_X', v_X: 'V_X' }[o.n];
  ...
  const p3sol = [
    ...
    ['iarr', [3, 2.5], 'd', { n: 'I_{D4}', s: '0.5' + mA, side: 'r' }],
    ['iarr', [3, 3.35], 'd', { n: 'I_{R_1}', s: '0.25' + mA, side: 'l', off: [-14, 0] }],
  ];
...
!!trap Points lost on the graded copy (−7.5)
- open-/short-circuit case not named (−1.5)
- $V_{OC}$ and $I_{SC}$ not scaled to $V_1 = 3\,\text{V}$ (−3)
- $\VTH = 2V_1$ not stated (−2)
- wrong route to $V_2$ (−1)`,
        figs: {
...
Full marks on the graded copy.`,
        figs: { ss: { fig: DRAW.ss(FIGS.shuntX(), { side: 'l' }), cap: 'incremental model' } },
      },
      {
...
!!trap Points lost on the graded copy (−10)
Writing $A_v = r_x/R_1$ instead of the divider made both $V_{OUT}$ and $V_{IN}$ wrong (−5, −5). Read the gain off the drawn incremental model.`,
...
[[fig:q]]
*$M_3$ and $M_4$ were printed as $1X$ and corrected to $\tfrac12X$ during the exam.*`,
...
[[fig:sol]]
Full marks on the graded copy: every $V_{GS}$, the KCL and the saturation inequality were written out.`,
      figs: {
        q: { fig: p3 },
